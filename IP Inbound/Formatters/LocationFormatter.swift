...
public struct CoordinateFormatStyle: FormatStyle, Codable, Equatable, Hashable {
    ...
    private func formatDegreesDecimalMinutes(_ coordinate: Coordinate) -> String {
        ...
        return String(
            localized: "\(northing) \(latDegrees, format: precision.intFormat)° \(latMinutes, format: precision.minuteDecimalFormat)′\n\(easting) \(lonDegrees, format: precision.intFormat)° \(lonMinutes, format: precision.minuteDecimalFormat)′"
        )
    }
    ...
    private func formatDegreesMinutesSeconds(_ coordinate: Coordinate) -> String {
        ...
        return String(
            localized: "\(northing) \(latDegrees, format: precision.intFormat)° \(latMinutes, format: precision.intFormat)′ \(latSeconds, format: precision.secondDecimalFormat)″\n\(easting) \(lonDegrees, format: precision.intFormat)° \(lonMinutes, format: precision.intFormat)′ \(lonSeconds, format: precision.secondDecimalFormat)″"
        )
    }
...
public struct CoordinateParseStrategy: ParseStrategy {
    ...
    private func parseDecimalDegrees(_ value: String) throws -> Coordinate {
        ...
        guard let match = regex.firstMatch(in: value, range: NSRange(location: 0, length: value.count)) else {
        ...
        guard let lat = Double(value[latRange]),
              let lon = Double(value[lonRange]) else {
    ...
    private func parseDegreesDecimalMinutes(_ value: String) throws -> Coordinate {
        ...
        guard let latDeg = Double(value[latDegRange]),
              ...
              let lonMin = Double(value[lonMinRange]) else {
            throw ParseError.invalidNumber
        }
    ...
    private func parseDegreesMinutesSeconds(_ value: String) throws -> Coordinate {
        ...
        let pattern = #"([NS])\s*(\d+)°?\s*(\d+)[′']?\s*([\d.]+)[″"]?\s*([EW])\s*(\d+)°?\s*(\d+)[′']?\s*([\d.]+)[″"]?"#
        let regex = try NSRegularExpression(pattern: pattern, options: .caseInsensitive)
        ...
        guard let latDeg = Double(value[latDegRange]),
              ...
              let lonSec = Double(value[lonSecRange]) else {
            throw ParseError.invalidNumber
        }
    ...
    enum ParseError: LocalizedError {
        ...
        var errorDescription: String? {
            switch self {
                case .invalidFormat:
                    String(localized: "coordinate.parse.error.format", defaultValue: "Invalid coordinate format")
                case .invalidNumber:
...
public extension Coordinate {
    ...
    init(_ string: String, format: CoordinateFormat) throws {
    ...
    func formatted(_ style: CoordinateFormatStyle) -> String {
        style.format(self)
    }
}
...
public extension FormatStyle where Self == CoordinateFormatStyle {
    ...
    static func coordinate(_ format: CoordinateFormat, precision: CoordinateFormatStyle.Precision = .standard) -> CoordinateFormatStyle {
