...
final class Geomagnetism {
  ...
  private static var WMM_COF: [String] {
    ...
    return String(data: data, encoding: .ascii)!.split(separator: .newlineSequence).map {
      String($0)
    }
  }
  ...
  func calculate(longitude: Double, latitude: Double, altitude: Double? = 0, date: Date? = Date()) {
    ...
    let crlon: Double = cos(rlon)
    let crlat: Double = cos(rlat)
    let srlat2: Double = srlat * srlat
    let crlat2: Double = crlat * crlat
    let a2: Double = Self.WGS84_A * Self.WGS84_A
    let b2: Double = Self.WGS84_B * Self.WGS84_B
    let c2: Double = a2 - b2
    let a4: Double = a2 * a2
    let b4: Double = b2 * b2
    let c4: Double = a4 - b4
    ...
    if altitudeKm != oalt || latitude != olat {
      ...
      let q1: Double = altitudeKm * q
      let q2: Double = ((q1 + a2) / (q1 + b2)) * ((q1 + a2) / (q1 + b2))
      let r2: Double = ((altitudeKm * altitudeKm) + 2 * q1 + (a4 - c4 * srlat2) / (q * q))
