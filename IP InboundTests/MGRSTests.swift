...
struct MGRSTests {
    ...
    func testCoordinateToMGRS() throws {
        ...
        for testCase in testCases {
            ...
            let mgrs = try #require(MGRSHelper.fromCoordinate(coordinate, precision: .tenM),
                                    "MGRS conversion failed for lat: \(testCase.lat), lon: \(testCase.lon)")
            #expect(mgrs.hasPrefix(testCase.expectedPrefix),
    ...
    func testMGRSToCoordinate() throws {
        ...
        for testCase in testCases {
            let coordinate = try #require(MGRSHelper.toCoordinate(testCase.mgrs),
                                         "Failed to parse MGRS: \(testCase.mgrs)")
            #expect(coordinate.latitudeDeg.isApproximatelyEqual(to: testCase.expectedLat, absoluteTolerance: testCase.tolerance),
                   "Latitude mismatch for MGRS: \(testCase.mgrs)")
            #expect(coordinate.longitudeDeg.isApproximatelyEqual(to: testCase.expectedLon, absoluteTolerance: testCase.tolerance),
    ...
    func testMGRSFormatting() {
        ...
        for testCase in testCases {
            ...
            #expect(formattedWithSpaces == testCase.withSpaces,
                   "Formatting with spaces failed for: \(testCase.input)")
            #expect(formattedWithoutSpaces == testCase.withoutSpaces,
    ...
    func testMGRSRoundTrip() throws {
        ...
        for original in coordinates {
            let mgrs = try #require(MGRSHelper.fromCoordinate(original, precision: .oneM),
                                   "Failed to convert coordinate to MGRS")
            let converted = try #require(MGRSHelper.toCoordinate(mgrs),
                                        "Failed to convert MGRS back to coordinate")
            #expect(original.latitudeDeg.isApproximatelyEqual(to: converted.latitudeDeg, absoluteTolerance: 0.00001),
                   "Latitude changed after round trip")
            #expect(original.longitudeDeg.isApproximatelyEqual(to: converted.longitudeDeg, absoluteTolerance: 0.00001),
    ...
    func testMGRSPrecisionLevels() throws {
        ...
        for test in precisionTests {
            let mgrs = try #require(MGRSHelper.fromCoordinate(coordinate, precision: test.precision),
                                   "Failed to generate MGRS at precision: \(test.precision)")
            let cleanMGRS = mgrs.replacingOccurrences(of: " ", with: "")
            #expect(cleanMGRS.count == test.expectedLength,
