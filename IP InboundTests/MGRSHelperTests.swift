    ...
    func testCoordinateConversion() throws {
        ...
        if let mgrs = MGRSHelper.fromCoordinate(oslo, precision: .hundredM),
           let backToCoord = MGRSHelper.toCoordinate(mgrs) {
            #expect(backToCoord.latitudeDeg.isApproximatelyEqual(to: oslo.latitudeDeg, absoluteTolerance: 0.01))
            #expect(backToCoord.longitudeDeg.isApproximatelyEqual(to: oslo.longitudeDeg, absoluteTolerance: 0.01))
        }
    }
