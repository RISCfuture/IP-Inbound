...
enum OffsetBearing: Hashable, Sendable {
  ...
  var degrees: Double { angle.degrees }
  ...
  init(degrees: Double, isTrue: Bool) {
  ...
  func toTrue(variation: Measurement<UnitAngle>) -> TrueBearing {
    switch self {
  ...
  func toMagnetic(variation: Measurement<UnitAngle>) -> MagneticBearing {
    switch self {
