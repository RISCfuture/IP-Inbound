...
public struct GuidanceHelper<T: GuidanceTarget> {
  ...
  /// The phase the last fix with geometry to solve put the aircraft in, or `nil` when this is the
  /// first fix of a run.
  ...
  ///   - previousGuidance: the phase the last fix with geometry to solve put the aircraft in, which
  ///     the early-arrival threshold widens against so a lead hovering at the boundary cannot flip
  ///     the phase at fix rate. A fix that solves to nothing has no phase of its own to offer: pass
  ///     the last one that did, so a dropout cannot drop a hold-off already under way.
